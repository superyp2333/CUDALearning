...
* **block（线程块）**：多个 thread 构成一个 block
  ...
  * 规则：1 个 block 中最多有 1024 个 thread，1 个 block 永远绑定 1 个 SM，1 个 SM 可以调度多个 Block
...
⚠️ 注意：block 和 grid 可以是多维的，这个“多维”只是逻辑划分，本质是为了降低 CUDA 编程的复杂性，**而对一维线程空间的逻辑封装**

> 例如：启动一个核函数的时候，需要指定 grid_size 和 block_size
>
> grid_size(2, 3)，则代表这个 grid 中有 6 个 block
>
> block_size(2, 3)，则代表这个 block 中有 6 个 thread
